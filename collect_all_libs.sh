...
ldd "$TARGET" | awk '{print $3}' | grep -v '^$' | grep -E 'Qt|av|sw|ndi|SDL' | grep -v -E 'cairo|pango|gtk|gdk|glib|gio|gobject|harfbuzz|vulkan|avahi|json-glib|graphene|libc\+\+' | while read lib; do
    if [ -f "$lib" ]; then
        cp -Lv "$lib" "$DEST/"
